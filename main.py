...
class Patient(BaseModel):
     id : Annotated[str, Field(..., description='ID of the patient in the DB.', examples=['P001'])]
...
def sort_patients(sort_by: str = Query(..., description='Sort on the basis of height, weight or bmi'), order:str = Query('asc',description='Sort in asc or desc order')):
     ...
     if order not in ['asc', 'desc']:
          raise HTTPException(status_code=400, detail='Invalid order. Please select asc or desc')
...
def create_patient(patient: Patient):
    ...
    return JSONResponse(status_code=201, content={'message': 'Patien created successfully', 'patient_id': patient.id})
